module buidlland::project {
    ...
    public entry fun create_project(
        ...
        let project_storage = borrow_global_mut<ProjectStorage>(@buidlland);
        ...
        project_storage.projects.add(project_id, project);
        // Is this the first time you create a project
        if (!project_storage.users_created_projects.contains(signer::address_of(account))) {
            let project_list = smart_vector::new<address>();
            project_list.push_back(project_id);
            project_storage.users_created_projects.add(signer::address_of(account), project_list);
        } else {
            // This is not the first time you create a project
            let project_list = project_storage.users_created_projects.borrow_mut(signer::address_of(account));
            project_list.push_back(project_id);
        }
    }
    ...
    public entry fun update_project(
        ...
        let project = project_storage.projects.borrow_mut(project_id);
    ...
    fun project_exists(
        ...
        project_storage.projects.contains(project_id)
    }

    // Check if the caller is the project creator
    public(package) fun is_project_creator_external(
        account: &signer,
        project_id: address
    ): bool acquires ProjectStorage {
        let project_storage = borrow_global<ProjectStorage>(@buidlland);
        let project_creators = project_storage.users_created_projects.borrow(signer::address_of(account));
        project_creators.contains(&project_id)
    }

    // Check if a project exists (external)
    public(package) fun project_exists_external(
        project_id: address
    ): bool acquires ProjectStorage {
        let project_storage = borrow_global<ProjectStorage>(@buidlland);
        project_storage.projects.contains(project_id)
    ...
    public fun get_project(
        ...
        let project = project_storage.projects.borrow(project_id);
    ...
    public fun get_user_created_projects(
        ...
        let project_ids = project_storage.users_created_projects.borrow(user);
        // Convert SmartVector to regular vector
        let user_created_projects_id = project_ids.to_vector();
    ...
    public fun get_total_projects_count(): u64 acquires ProjectStorage {
        ...
        project_storage.projects.length()
    }
}